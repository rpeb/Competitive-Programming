...
#define FR(i, L, R) for (int i = L; i*i <= R; i++)
...
#define wez(n) \
  int(n);      \
  scanf("%d", &(n)) //handy if the input is right after the definition of a variable
#define wez2(n, m) \
  int(n), (m);     \
  scanf("%d %d", &(n), &(m))
#define wez3(n, m, k) \
...
#define IOS                     \
  ...
  cin.tie(NULL) //to synchronize the input of cin and scanf
